...
class SocietyAddAdminManagementPage extends StatefulWidget {
  Map data;

  SocietyAddAdminManagementPage(this.data);

  @override
  _SocietyAddAdminManagementPageState createState() => _SocietyAddAdminManagementPageState();
}

class _SocietyAddAdminManagementPageState extends State<SocietyAddAdminManagementPage> {
  List list = [];
  bool allCheck = false;
  ...
  void initState() {
    super.initState();
    Bus.sub(BUS_SOCIETY_ADMIN_MANAGEMENT_DATA, (data) {
      setState(() {
        list = data;
      });
    });
  ...
  void dispose() {
    super.dispose();
    Bus.fire(BUS_SOCIETY_ADMIN_MANAGEMENT_DATA);
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: xAppBar('新增管理员', action: [
          AppTextButton(
            title: Text('全选', style: TextStyle(color: AppPalette.hint, fontSize: 14)),
            width: 50,
            onPress: () {
              allCheck = !allCheck;
              Bus.send(BUS_SOCIETY_ADMIN_MANAGEMENT_ALL, allCheck);
            },
          ),
          AppTextButton(
              title: Text('新增', style: TextStyle(color: AppPalette.primary, fontSize: 14)),
              width: 50,
              ...
              onPress: () {
                ...
                simpleSub(Api.Family.setupAdministrator(familyId: widget.data['familyId'].toString(), userIds: userIds),
                    callback: () {
                  Bus.send(BUS_SOCIETY_ADMIN_MANAGEMENT_REFRESH);
                  Bus.send(BUS_SOCIETY_ADMIN_MANAGEMENT_ALL, allCheck);
                });
              })
        ]),
        body: Material(
          color: Colors.white,
          child: SizedBox(
              height: double.infinity, width: double.infinity, child: SingleChildScrollView(child: societyMember())),
        ));
  }
...
class _SocietyMemberListState extends NetPageList<dynamic, SocietyMemberList> {
  ...
  void initState() {
    super.initState();
    Bus.sub(BUS_SOCIETY_ADMIN_MANAGEMENT_ALL, (data) {
      listData.forEach((element) {
        element['app_check'] = data;
      });
      Bus.send(BUS_SOCIETY_ADMIN_MANAGEMENT_DATA, listData);
      setState(() {});
    });
    Bus.sub(BUS_SOCIETY_ADMIN_MANAGEMENT_REFRESH, (data) {
      doRefresh();
    });
  }
  ...
  void dispose() {
    super.dispose();
    Bus.fire(BUS_SOCIETY_ADMIN_MANAGEMENT_ALL);
  }

...
class SocietyAdminManagementPage extends StatefulWidget {
  Map data;

  SocietyAdminManagementPage(this.data);

  @override
  _SocietyAdminManagementPageState createState() => _SocietyAdminManagementPageState();
}

class _SocietyAdminManagementPageState extends State<SocietyAdminManagementPage> {
  List list = [];
  bool allCheck = false;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        appBar: xAppBar('管理员管理', action: [
          ...
          AppTextButton(
              title: Text('移除', style: TextStyle(color: AppPalette.pink, fontSize: 14)),
              width: 50,
              onPress: () {
                if (list == null || list.length == 0) return;
                List userIds = List();
                for (dynamic data in list) {
                  if (data['app_check'] ?? false) userIds.add(data);
                }
                simpleSub(
                    Api.Family.removeAdmin(
                        familyId: widget.data['familyId'].toString(),
                        userIds: userIds.map((e) => e['uid']).toList()), callback: () {
                  Bus.send(BUS_SOCIETY_ADMIN_MANAGEMENT_REFRESH);
                  Bus.send(BUS_SOCIETY_ADMIN_MANAGEMENT_ALL, allCheck);
                  for (dynamic data in userIds) {
                    list.remove(data);
                  }
                });
              }),

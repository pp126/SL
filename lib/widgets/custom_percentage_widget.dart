...
class CustomCircularLoader extends StatefulWidget {
  CustomCircularLoader({
    ...
    this.circleHeaderStyle,
        ...
        coveredPercent = min(100, coveredPercent);
...
class _CircularLoader extends State<CustomCircularLoader> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    if (widget.circleStart == "top")
      circleStartPoint = 1.5;
    else if (widget.circleStart == "bottom")
      circleStartPoint = 0.5;
    else if (widget.circleStart == "right")
      circleStartPoint = 2;
    else if (widget.circleStart == "left")
      circleStartPoint = 1;
    else
      circleStartPoint = 1.5;
  ...
  Widget defaultContent() {
    Widget percentageText = Row(
      ...
      children: <Widget>[
        ...
        Text(widget.unit ?? "%", style: widget.coveredPercentStyle),
      ],
    );

    Widget headerText = (widget.circleHeader == null)
        ? Container()
        : Text(
            widget.circleHeader,
...
class GenericCircle extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final radius = min((size.width - circleWidth - variance) / 2, (size.height - circleWidth - variance) / 2);
    canvas.drawCircle(center, radius, circle); //Circle drawn.....

    final arcAngle = otherVal != null ? 2 * pi * 0 : 2 * pi * (coveredPercent / 100);

    canvas.drawArc(
        new Rect.fromCircle(center: center, radius: radius), pi * circleStartPoint, arcAngle, false, coveredPath);
  }
